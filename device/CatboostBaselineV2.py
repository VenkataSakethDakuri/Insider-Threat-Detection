...
pc_freq = df['pc'].value_counts()
df['rare_pc_flag'] = df['pc'].map(pc_freq) < 10
...
feature_cols = [
    ...
    'is_after_hours', 'connect_ratio', 'is_weekend',
...
categorical_features_indices = [feature_cols.index('activity_encoded'), feature_cols.index('role_encoded'), feature_cols.index('rare_pc_flag'), feature_cols.index('rare_hour_for_user'), feature_cols.index('is_weekend'), feature_cols.index('dayofweek')]
...
# 6. Ensemble Model (CatBoost + RF voting)
