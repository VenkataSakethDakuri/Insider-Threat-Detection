...
df_train['rolling_50_anomaly'] = df_train.groupby('user')['label'].rolling(50, min_periods=1).mean().reset_index(level=0, drop=True).shift(1)
...
# Initialize dictionaries to track connects per hour
user_hour_connects = {}  # {user: {hour_timestamp: connect_count}}
...
for i in range(len(df_test)):
    ...
    # Update connects_per_hour for next iteration (only if it's a connect activity)
    if row['activity_encoded'] == 1:
        user_hour_connects[user][hour_ts] = connects_in_hour + 1
