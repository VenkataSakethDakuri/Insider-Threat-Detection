...
# OPTIMIZATION 1: Pre-cache user histories from training data
user_hist_cache = {user: df_train[df_train['user'] == user] for user in df_train['user'].unique()}

for i in range(len(df_val)):
    idx = df_val.index[i]
    user = df_val.at[idx, 'user']
    pc = df_val.at[idx, 'pc']
    
    # OPTIMIZATION 2: Use cached history
    hist = user_hist_cache.get(user, pd.DataFrame())
    user_full = pd.concat([hist, df_val.iloc[[i]]]).sort_values('Timestamp')
    
    expanding_mean = user_full['time_since_last'].expanding().mean().loc[idx]
    expanding_std = user_full['time_since_last'].expanding().std().loc[idx]
    expanding_std = 1 if expanding_std == 0 or pd.isna(expanding_std) else expanding_std
    
    df_val.at[idx, 'user_gap_mean'] = expanding_mean
    df_val.at[idx, 'user_gap_std'] = expanding_std
    df_val.at[idx, 'gap_zscore'] = (df_val.at[idx, 'time_since_last'] - expanding_mean) / expanding_std

    pc_freq[pc] = pc_freq.get(pc, 0) + 1
    df_val.at[idx, 'rare_pc_flag'] = int(pc_freq[pc] < 10)
    
    hour_count = user_full['hour'].value_counts().get(user_full.loc[idx, 'hour'], 0)
    df_val.at[idx, 'rare_hour_for_user'] = int(hour_count < 3)
...
# OPTIMIZATION 3: Pre-build test graph and cache histories
G_test = nx.Graph()
combined_train_val = pd.concat([df_train, df_val], ignore_index=False)
G_test.add_edges_from(combined_train_val[['user', 'pc']].drop_duplicates().values.tolist())

# OPTIMIZATION 4: Pre-cache user histories for test (includes train + val)
user_hist_test_cache = {user: combined_train_val[combined_train_val['user'] == user] 
                        for user in combined_train_val['user'].unique()}

# Initialize all feature columns for df_test
for col in ['user_degree', 'pc_degree', 'rolling_3_anomaly', 'rolling_10_anomaly', 
            'rolling_20_anomaly', 'anomaly_momentum', 'rare_pc_flag', 'rare_hour_for_user',
            'user_gap_mean', 'gap_zscore']:
    df_test[col] = 0.0
...
# OPTIMIZATION 5: Pre-compute feature matrix template
feature_replacements = {np.inf: 0, -np.inf: 0}

# OPTIMIZATION 6: Use itertuples for faster iteration
test_indices = df_test.index.tolist()

for i in range(len(df_test)):
    idx = test_indices[i]
    row = df_test.loc[idx]
    user, pc = row['user'], row['pc']
    
    # Use cached history
    hist = user_hist_test_cache.get(user, pd.DataFrame())
    user_full = pd.concat([hist, df_test.loc[[idx]]]).sort_values('Timestamp')
    
    # Compute rolling anomaly features more efficiently
    label_series = user_full['label']
    for w in [3, 10, 20]:
        val = label_series.rolling(w, min_periods=1).mean().shift(1).loc[idx]
        df_test.at[idx, f'rolling_{w}_anomaly'] = val if not pd.isna(val) else 0.0
    
    # Compute expanding statistics
    time_series = user_full['time_since_last']
    expanding_mean = time_series.expanding().mean().loc[idx]
    expanding_std = time_series.expanding().std().loc[idx]
    expanding_std = 1 if expanding_std == 0 or pd.isna(expanding_std) else expanding_std
    
    df_test.at[idx, 'user_gap_mean'] = expanding_mean
    df_test.at[idx, 'user_gap_std'] = expanding_std
    df_test.at[idx, 'gap_zscore'] = (row['time_since_last'] - expanding_mean) / expanding_std
    df_test.at[idx, 'anomaly_momentum'] = df_test.at[idx, 'rolling_10_anomaly'] - df_test.at[idx, 'rolling_20_anomaly']

    # Update pc frequency
    pc_freq[pc] = pc_freq.get(pc, 0) + 1
    df_test.at[idx, 'rare_pc_flag'] = int(pc_freq[pc] < 10)
    
    # Compute rare hour
    hour_count = user_full['hour'].value_counts().get(row['hour'], 0)
    ...
    # OPTIMIZATION 7: Inline prediction without extra array indexing
    X_row = df_test.loc[[idx], feature_cols].replace([np.inf, -np.inf], 0).fillna(0)
    prediction_proba = ensemble.predict_proba(X_row)[0, 1]
    y_test_pred_proba.append(prediction_proba)
    
    df_test.at[idx, 'label'] = 1 if prediction_proba >= best_thr else 0
