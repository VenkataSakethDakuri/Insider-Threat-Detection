...
# 1. Load Data
df = pd.read_csv('/home/gururaj/datasets/PROCESSED_CERT+ROLES/http_conv.csv') 
df['anomaly_status'] = df['anomaly_status'].replace({2: 1, 3: 1})
df['label'] = df['anomaly_status'].astype(int)

# 2. Parse Query for url/user/pc (ignore content)
QUOTE = r"['\u2018\u2019\"\u201C\u201D]"
def parse_http_query(query):
    if pd.isna(query):
        return {'url': None, 'user': None, 'pc': None}
    q = str(query)
    url = re.search(rf"SELECT\s*{QUOTE}(.+?){QUOTE}\s*AS\s*url", q)
...
parsed = df['Query'].apply(parse_http_query)
df['url']  = parsed.apply(lambda x: x['url'])
...
df = df.dropna(subset=['Timestamp', 'user', 'pc', 'role', 'url']).reset_index(drop=True)
...
url_encoder  = LabelEncoder(); df['url_encoded']  = url_encoder.fit_transform(df['url'])
...
df['rolling_10_count']   = df.groupby('user')['url'].rolling(10, min_periods=1).count().reset_index(level=0, drop=True)
df['rolling_100_count']  = df.groupby('user')['url'].rolling(100, min_periods=1).count().reset_index(level=0, drop=True)
...
df['rare_url_flag'] = df['url'].map(url_freq) < 10
...
feature_cols = [
    ...
    'user_encoded', 'pc_encoded', 'role_encoded', 'url_encoded',
    ...
    'rare_url_flag', 'rare_hour_for_user',
...
categorical_features_indices = [feature_cols.index(f) for f in [
    'user_encoded', 'pc_encoded', 'role_encoded', 'url_encoded', 'rare_url_flag', 'rare_hour_for_user', 'is_weekend', 'dayofweek'
]]
...
plt.title('Receiver Operating Characteristic (ROC) Curve (HTTP Events)')
plt.legend(loc="lower right")
plt.grid(alpha=0.3)
plt.savefig('roc_curve_http.png', dpi=300, bbox_inches='tight')
print("ROC curve saved as 'roc_curve_http.png'")
plt.show()
